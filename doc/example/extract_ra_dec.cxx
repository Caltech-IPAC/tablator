...
/// g++ extract_ra_dec.cxx -o extract_ra_dec -I/usr/include/hdf5/serial/ -std=c++11 -ltablator -lboost_filesystem -lboost_system
...
int main(int argc, char *argv[])
{
  ...
  for (size_t column=0; column<input_table.columns.size(); ++column)
    {
      if (input_table.columns[column].name=="ra")
        ra_index=column;
      if (input_table.columns[column].name=="dec")
        dec_index=column;
    }
  ...
  tablator::Table output_table(std::vector<tablator::Column>
                               ({input_table.columns[ra_index],
                                   input_table.columns[dec_index]}));
      ...
      if (input_table.is_null (row_offset, ra_index))
        {
          ...
          output_row.set_null (output_table.columns[1].type,
                               output_table.columns[1].array_size,
                               1, output_table.offsets[1],
      ...
      if (input_table.is_null (row_offset, dec_index))
        {
          output_row.set_null (output_table.columns[2].type,
                               output_table.columns[2].array_size,
                               2, output_table.offsets[2],
                               output_table.offsets[3]);
        }
      else
        {
          auto input_row_ptr=input_table.data.begin() + row_offset;
