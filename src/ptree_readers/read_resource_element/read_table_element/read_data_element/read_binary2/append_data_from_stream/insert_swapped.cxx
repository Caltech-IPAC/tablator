...
T parse_swapped_element(const Rule &rule,
...
void record_parsed_swapped_element(uint8_t *temp_ptr,
                                   std::vector<uint8_t>::const_iterator &src_begin,
                                   std::vector<uint8_t>::const_iterator &src_end,
    ...
    switch (data_type_size) {
        case 1:
            ...
            break;
        case 2:
            *(reinterpret_cast<uint16_t *>(temp_ptr)) = parse_swapped_element<uint16_t>(
                    boost::spirit::qi::big_word, src_begin, src_end);
            break;
        case 4:
            *(reinterpret_cast<uint32_t *>(temp_ptr)) = parse_swapped_element<uint32_t>(
                    boost::spirit::qi::big_word, src_begin, src_end);
            break;
        case 8:
            *(reinterpret_cast<uint64_t *>(temp_ptr)) = parse_swapped_element<uint64_t>(
                    boost::spirit::qi::big_word, src_begin, src_end);
            break;
        default:
...
void insert_swapped_array(tablator::Row &row, size_t column_offset,
                          ...
                          size_t index_in_dynamic_cols_list) {
    switch (data_type_size) {
        case 1: {
            row.insert(temp_vec.data(), temp_vec.data() + possibly_dynamic_array_size,
                       column_offset, index_in_dynamic_cols_list);
        } break;
...
void insert_swapped(Row &row, size_t column_offset, const Data_Type &data_type,
    ...
    // Swap endedness of array elements into temp_vec and then insert as a group into
    // row.
    ...
    for (size_t index = 0; index != possibly_dynamic_array_size; ++index) {
        ...
        record_parsed_swapped_element(curr_temp_ptr, src_begin, src_end, data_type_size,
                                      is_bool);
        curr_temp_ptr += data_type_size;
    }
    insert_swapped_array(row, column_offset, temp_vec, data_type_size,
                         possibly_dynamic_array_size, index_in_dynamic_cols_list);
}
}  // namespace tablator
