...
void insert_swapped_element(tablator::Row &row, size_t offset,
                            std::vector<uint8_t>::const_iterator &src_begin,
                            std::vector<uint8_t>::const_iterator &src_end,
    ...
    switch (data_type_size) {
        case 1:
            ...
            break;
        case 2:
            parse_and_insert_element<uint16_t>(row, offset, boost::spirit::qi::big_word,
                                               src_begin, src_end);
            break;
        case 4:
            parse_and_insert_element<uint32_t>(
                    row, offset, boost::spirit::qi::big_dword, src_begin, src_end);
            break;
        case 8:
            parse_and_insert_element<uint64_t>(
                    row, offset, boost::spirit::qi::big_qword, src_begin, src_end);
            break;
        default:
...
void insert_swapped(Row &row, size_t column_offset, const Data_Type &data_type,
    ...
    for (size_t index = 0; index != array_size; ++index) {
        ...
        insert_swapped_element(row, column_offset + index * data_type_size, src_begin,
                               src_end, data_type_size, is_bool);
    }
}
}  // namespace tablator
