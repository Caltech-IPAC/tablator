  ...
  try
    {
      boost::program_options::store
        (boost::program_options::command_line_parser
         (argc, argv).options(options).positional(positional_options).run(),
         option_variables);
      ...
      if (option_variables.count("output-format"))
        output_format.init (option_variables["output-format"].as<std::string>());

      boost::filesystem::path input_path, output_path;
      if (option_variables.count("files"))
        {
          std::vector<std::string> paths (option_variables["files"]
