...
size_t compute_max_column_width(const tablator::Table &table, size_t col_idx,
    ...
    uint8_t const *col_data_start_ptr = data_start_ptr + base_offset;
    uint8_t const *curr_col_data_ptr = col_data_start_ptr;

    for (size_t requested_row_id : requested_row_ids) {
        curr_row_start_offset = requested_row_id * table.row_size();
        curr_col_data_ptr = col_data_start_ptr + curr_row_start_offset;
...
size_t compute_max_column_width_for_double(
        const tablator::Table &table, size_t col_idx, size_t col_width_from_headers,
        const std::vector<size_t> &requested_row_ids) {
    size_t max_width_sofar = col_width_from_headers;
...
size_t compute_max_column_width_for_char(const tablator::Table &table, size_t col_idx,
                                         size_t array_size,
                                         size_t col_width_from_headers,
                                         const std::vector<size_t> &requested_row_ids) {
    size_t max_width_sofar = col_width_from_headers;
...
size_t tablator::Ipac_Table_Writer::get_single_column_width(
    ...
    switch (type) {
        ...
        case Data_Type::UINT8_LE: {
            ...
            max_width_sofar = std::max(max_width_sofar, MAX_INT8_STRLEN);
        } break;
        case Data_Type::INT16_LE: {
            // std::cout << "INT16" << std::endl;
            max_width_sofar = std::max(max_width_sofar, MAX_INT16_STRLEN);
        } break;
        case Data_Type::UINT16_LE: {
            // std::cout << "UINT16" << std::endl;
            max_width_sofar = compute_max_column_width<uint16_t>(
                    table, col_idx, max_width_sofar, requested_row_ids);
        } break;
        case Data_Type::INT32_LE: {
            // std::cout << "INT32" << std::endl;
            max_width_sofar = compute_max_column_width<int32_t>(
                    table, col_idx, max_width_sofar, requested_row_ids);
        } break;
        case Data_Type::UINT32_LE: {
            // std::cout << "UINT32" << std::endl;
            max_width_sofar = compute_max_column_width<uint32_t>(
                    table, col_idx, max_width_sofar, requested_row_ids);
        } break;
        case Data_Type::INT64_LE: {
            max_width_sofar = compute_max_column_width<int64_t>(
                    table, col_idx, max_width_sofar, requested_row_ids);
        } break;
        case Data_Type::UINT64_LE: {
            max_width_sofar = compute_max_column_width<uint64_t>(
                    table, col_idx, max_width_sofar, requested_row_ids);
        } break;
        case Data_Type::FLOAT32_LE: {
            max_width_sofar = compute_max_column_width<float>(
                    table, col_idx, max_width_sofar, requested_row_ids);
        } break;
        case Data_Type::FLOAT64_LE: {
        ...
        case Data_Type::CHAR: {
            max_width_sofar =
                    //	  std::max(max_width_sofar, column.get_array_size());
                    compute_max_column_width_for_char(
                            table, col_idx, column.get_array_size(), max_width_sofar,
                            requested_row_ids);
        } break;
    }
...
std::vector<size_t> tablator::Ipac_Table_Writer::get_column_widths(
    ...
    for (auto col_idx : sorted_col_ids) {
        ...
        if (is_valid_col_idx(table, col_idx)) {
            widths.push_back(
                    get_single_column_width(table, requested_row_ids, col_idx));
            ++prev_col_idx;
        } else {
...
void tablator::Ipac_Table_Writer::write_subtable_by_row(
    ...
    tablator::Ipac_Table_Writer::write_subtable_by_row(
            ...
            get_column_widths(table, requested_row_ids, get_all_nonzero_col_ids(table)),
            Data_Type_Adjuster(table).get_datatypes_for_writing(
                    Format::Enums::IPAC_TABLE),
            skip_comments);
}
...
void tablator::Ipac_Table_Writer::write_subtable_by_column_and_row(
    ...
    tablator::Ipac_Table_Writer::write_subtable_by_column_and_row(
            ...
            get_column_widths(table, requested_row_ids, column_ids),
            Data_Type_Adjuster(table).get_datatypes_for_writing(
                    Format::Enums::IPAC_TABLE),
            skip_comments);
}
...
void tablator::Ipac_Table_Writer::write_subtable_by_row(
        ...
        size_t requested_consecutive_row_count, bool skip_comments) {
    ...
    tablator::Ipac_Table_Writer::write_subtable_by_row(
            table, os, start_row, true_row_count,
            get_column_widths(table, requested_row_ids, get_all_nonzero_col_ids(table)),
            Data_Type_Adjuster(table).get_datatypes_for_writing(
                    Format::Enums::IPAC_TABLE),
            skip_comments);
...
void tablator::Ipac_Table_Writer::write_subtable_by_column_and_row(
        const Table &table, std::ostream &os, const std::vector<size_t> &column_ids,
        size_t start_row, size_t requested_consecutive_row_count, bool skip_comments) {
    size_t true_row_count =
            get_true_row_count(table, start_row, requested_consecutive_row_count);
