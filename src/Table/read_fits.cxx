...
read_keywords_as_labeled_properties(CCfits::ExtHDU &table_extension) {
    ...
    for (auto &kwd : sorted_kwd_map) {
        ...
        if (kwd_word->keytype() == CCfits::Tstring) {
            ...
            if (std::regex_search(kwd_value, label_match, label_expr)) {
                prop_identifier.assign(label_match[1]);
                name.assign(boost::to_lower_copy(prop_identifier));

                prop_label.assign(prop_identifier);
                kwd_value.assign((label_match[2]));

                std::smatch attr_match;
                if (std::regex_search(prop_identifier, attr_match, attr_expr)) {
                    ...
                    prop_identifier.assign(attr_match[1]);
                    prop_label.assign(prop_identifier);

                    name.assign(attr_match[2]);
                    if (!boost::equals(name, tablator::ID)) {
                    ...
                    if (std::regex_search(prop_identifier, info_match, info_expr)) {
                        prop_label.assign(info_match[1]);
                    } else {
