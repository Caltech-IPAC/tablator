  ...
  for(auto &c: table->column ())
    {
      ...
      switch (c.second->type())
        ...
        case CCfits::Tlogical:
          ...
          read_column<char>(current_row+offset,c,table->rows());
          break;
        case CCfits::Tstring:
          ...
          {
            std::vector<valarray<char> > v;
            c.second->readArrays(v,1,table->rows());
            for(auto &element: v)
