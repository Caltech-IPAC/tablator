...
void tablator::Table::read_ipac_table(std::istream &input_stream) {
    ...
    const auto incoming_column_widths =
            get_incoming_column_widths(incoming_column_offsets);
    ...
    while (input_stream) {
        if (line.find_first_not_of(" \t") != std::string::npos) {
            for (size_t col_idx = 1; col_idx < num_tablator_columns; ++col_idx) {
                ...
                std::string element =
                        line.substr(incoming_column_offsets[col_idx - 1] + 1,
                                    incoming_column_widths[col_idx]);
                boost::algorithm::trim(element);
                if (tab_column.get_type() == Data_Type::CHAR &&
                    element.size() != max_column_widths_sofar[col_idx]) {
    ...
    shrink_ipac_string_columns_to_fit(tab_columns, offsets, max_column_widths_sofar);
