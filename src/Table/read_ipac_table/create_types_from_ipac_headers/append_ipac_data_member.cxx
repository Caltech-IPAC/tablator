...
void tablator::Table::append_ipac_data_member(const std::string &name,
    ...
    if (std::string("boolean").compare(0, string_size, t) == 0) {
        ...
        append_column(columns, offsets, name, Data_Type::UINT8_LE);
    } else if (std::string("short").compare(0, string_size, t) == 0) {
        append_column(columns, offsets, name, Data_Type::INT16_LE);
    } else if (std::string("ushort").compare(0, string_size, t) == 0) {
        append_column(columns, offsets, name, Data_Type::UINT16_LE);
    } else if (std::string("int").compare(0, string_size, t) == 0) {
        append_column(columns, offsets, name, Data_Type::INT32_LE);
    } else if (std::string("uint").compare(0, string_size, t) == 0) {
        append_column(columns, offsets, name, Data_Type::UINT32_LE);
    } else if (std::string("long").compare(0, string_size, t) == 0) {
        append_column(columns, offsets, name, Data_Type::INT64_LE);
    } else if (std::string("ulong").compare(0, string_size, t) == 0) {
        append_column(columns, offsets, name, Data_Type::UINT64_LE);
    } else if (std::string("float").compare(0, string_size, t) == 0) {
        append_column(columns, offsets, name, Data_Type::FLOAT32_LE);
    } else if (std::string("double").compare(0, string_size, t) == 0 ||
               std::string("real").compare(0, string_size, t) == 0) {
        append_column(columns, offsets, name, Data_Type::FLOAT64_LE);
    } else if (std::string("char").compare(0, string_size, t) == 0 ||
               std::string("date").compare(0, string_size, t) == 0) {
        append_column(columns, offsets, name, Data_Type::CHAR, num_elements);
    } else {
