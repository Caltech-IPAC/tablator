...
void tablator::Table::shrink_ipac_string_columns_to_fit(
    ...
    std::vector<std::vector<uint32_t>> &old_dynamic_array_sizes_by_row =
            old_data_details.get_dynamic_array_sizes_by_row();
    ...
    Data_Details new_data_details(old_field_framework.get_dynamic_column_ids(),
                                  new_row_size, num_rows);
    std::vector<uint8_t> &new_data = new_data_details.get_data();

    std::vector<std::vector<uint32_t>> &new_dynamic_array_sizes_by_row =
            new_data_details.get_dynamic_array_sizes_by_row();
    ...
    swap(old_field_framework, new_field_framework);

    swap(old_data_details, new_data_details);
}
