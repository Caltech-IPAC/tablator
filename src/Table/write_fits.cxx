...
void write_column(fitsfile *fits_file, const int &fits_type, const int &column,
                  const uint8_t *data, const hsize_t &array_size, const size_t &row) {
    int status = 0;
  ...
  for (size_t i = 0; i < columns.size (); ++i)
    {
      ...
      char fits_type;
          ...
          /// both it and signed long as longlong and hope for
          /// the best.
          fits_type = 'K';
          break;
          ...
          const uint8_t *offset_data = row_pointer + offsets[i];
            switch (column.type) {
                ...
                case Data_Type::FLOAT64_LE:
                    ...
                    break;
                case Data_Type::CHAR: {
                    std::string temp_string(reinterpret_cast<const char *>(offset_data),
