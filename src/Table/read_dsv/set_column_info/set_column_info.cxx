...
void
tablator::Table::set_column_info (std::list<std::vector<std::string> > &dsv)
{
  auto row (dsv.begin ());
  ...
  for (; row != dsv.end (); ++row)
    {
      if (row->size () != names.size ())
        throw std::runtime_error (
            "In line " + std::to_string (line_number) + ", expected "
            + std::to_string (names.size ()) + " elements, but only found "
