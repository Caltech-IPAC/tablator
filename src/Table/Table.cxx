...
tablator::ATTRIBUTES add_column_elements_to_attributes(tablator::Column column) {
    auto attrs = column.get_field_properties().get_attributes();
    attrs.emplace(std::make_pair("datatype",
                                 std::to_string(static_cast<int>(column.get_type()))));
...
void append_column_attributes_with_label(
    ...
    for (const auto &column : column_list) {
        const auto attrs = add_column_elements_to_attributes(column);
        if (attrs.empty()) {
            // shouldn't happen
            return;
        }
        combined_list.emplace_back(
                std::make_pair(partial_label + column.get_name(), attrs));
    }
}
...
bool Table::stash_attributes_labeled_by_element(
    ...
    if (boost::equals(label, VOTABLE_RESOURCE_XMLATTR)) {
        ...
        // e.g. FIELD or PARAM,  handled elsewhere
        return false;
    }
