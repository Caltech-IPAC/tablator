...
std::pair<size_t, tablator::Format_Packet> get_offsets_and_types(
    ...
    return std::pair<size_t, tablator::Format_Packet>(
            table.get_offsets().at(std::distance(columns.begin(), column)),
            column->get_format_packet());
}

std::pair<std::pair<size_t, tablator::Format_Packet>,
...
void tablator::Table::write_sql_inserts(
    ...
    std::pair<std::pair<size_t, Format_Packet>, std::pair<size_t, Format_Packet>>
            point_input;
