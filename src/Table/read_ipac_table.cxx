...
{
  size_t ncols = 0, file_size = 0, ipac_row_size{ 0 }, ipac_row_number{ 0 },
         ipac_data_offset = 0;
  ...
  while (std::getline (ifs, str))
    {
      ...
      if (str.find ("\\ ") == 0)
      ...
      else if (str.find ("\\") == 0)
        {
          ...
          key = boost::algorithm::trim_copy (
              str.substr (1, position_of_equal - 1));
          val = boost::algorithm::trim_copy (
              str.substr (position_of_equal + 1));
          ...
          if (header == 1)
            {
              ...
              boost::split (names,
                            str.erase (str.find_last_of ("|"), 1).erase (0, 1),
                            boost::is_any_of ("|"));
          ...
          else if (header == 2)
            {
              if (!check_bar_position (ipac_column_offset, str))
                throw std::runtime_error (
                    "Mis-locate the bar(|) in the datatype header.");
              boost::split (dtypes,
                            str.erase (str.find_last_of ("|"), 1).erase (0, 1),
                            boost::is_any_of ("|"));
          ...
          else if (header == 3)
            {
              if (!check_bar_position (ipac_column_offset, str))
                throw std::runtime_error (
                    "Mis-locate the bar(|) in the unit header.");
              boost::split (units,
                            str.erase (str.find_last_of ("|"), 1).erase (0, 1),
                            boost::is_any_of ("|"));
            }
          else if (header == 4)
            {
              if (!check_bar_position (ipac_column_offset, str))
                throw std::runtime_error (
                    "Mis-locate the bar(|) in the null header.");
              boost::split (nulls,
                            str.erase (str.find_last_of ("|"), 1).erase (0, 1),
                            boost::is_any_of ("|"));
              for (auto &n : nulls)
  ...
  if (fixlen_key)
    {
      if (!fixlen_table)
        throw std::runtime_error (
            "Not a fixlen table although 'fixlen = T' is defined.");
  ...
  else
    {
      ...
      while (std::getline (ifs, str))
        {
          if (str.size ()
              <= static_cast<size_t>(ipac_column_offset[ncols - 1] + 2))
            throw std::runtime_error ("Bad formatted IPAC table.");
          ipac_row_offset.push_back (str.size () - ipac_column_offset[ncols]
                                     - 1);
          ++ipac_row_number;
        }
      ifs.clear ();
  ...
  for (size_t i = 0; i < ncols; ++i)
    {
      ...
      switch (types[i])
        ...
        case Type::STRING:
          ...
          row_size += H5::PredType::NATIVE_CHAR.getSize ()
                      * (ipac_column_widths[i]);
          break;
  ...
  for (size_t i = 0; i < ncols; ++i)
    {
      switch (types[i])
        ...
        case Type::DOUBLE:
        case Type::FLOAT:
          compound_type.insertMember (names[i], offsets[i],
