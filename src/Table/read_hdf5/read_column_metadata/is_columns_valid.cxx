...
bool is_columns_valid (H5::VarLenType &columns)
{
  ...
  if (!(column.getNmembers () == 4
        && column.getMemberClass (0) == H5T_STRING
  ...
  H5::DataType
    attributes_super (field_properties.getMemberVarLenType (1).getSuper ());
  if (attributes_super.getClass () != H5T_COMPOUND
      || !is_key_value_type (H5::CompType (attributes_super.getId ())))
    { return false; }

  H5::DataType
    links_super (field_properties.getMemberVarLenType (1).getSuper ());
