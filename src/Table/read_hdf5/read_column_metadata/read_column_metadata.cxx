  ...
  for (size_t column = 0; column < hdf5_columns.len; ++column)
    {
      ...
      HDF5_Field_Properties
        &hdf5_field_properties (hdf5_column.field_properties);
      ...
      for (size_t attribute=0; attribute < hdf5_attributes.len; ++attribute)
        {
          HDF5_Attribute &a (reinterpret_cast<HDF5_Attribute *>
                             (hdf5_attributes.p)[attribute]);
          const std::string name (a.name);
          if (attributes.find (name) != attributes.end ())
            {
              throw std::runtime_error ("In " + section
                                        + " in column " + hdf5_column.name
      ...
      for (size_t option=0; option < hdf5_options.len; ++option)
        {
          HDF5_Attribute
            &o (reinterpret_cast<HDF5_Attribute *> (hdf5_options.p)[option]);
      ...
      Field_Properties field_properties (hdf5_field_properties.description,
                                         attributes, values, links);
      result.emplace_back (hdf5_column.name,
                           string_to_Data_Type (hdf5_column.type),
