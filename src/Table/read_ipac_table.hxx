...
inline bool 
check_bar_position(std::vector<int> &off, std::string &str)
  {
    for (size_t i=0; i < off.size()-1; i++)
       if (str.at(off[i]) != '|') return false;  
    return true;
  }

inline std::vector<int> 
get_ipac_column_widths(std::vector<int> &off)
  {
...
inline std::vector<int> 
find_ipac_column_offset(std::string &str, char find)
  {
    std::vector<int> ipac_column_offset;
    for(size_t i =0; i < str.size(); i++)
        if(str[i] == find) ipac_column_offset.push_back(i);
    return ipac_column_offset;
  }

inline bool validate_ipac_header 
    ...
    for (auto &v: vec)
      {
        ...
        if (!(good_headerline = (regex_match(v, vec_regex))))
            throw std::runtime_error( "Column name/datatype '" + v+"' contains an invalid character.");
      }
...
assign_data_type (std::vector<std::string> &vec) 
  {
    ...
    for (auto &t: vec)
      {
        if (boost::iequals(t,"double") ||
            boost::iequals(t,"float")  ||
        ...
        if (boost::iequals(t,"int") ||
            boost::iequals(t,"integer"))
...
void trim(char *str) {
    ...
    if (str != p) memmove(str, p, strlen(p)+1);
}
}
