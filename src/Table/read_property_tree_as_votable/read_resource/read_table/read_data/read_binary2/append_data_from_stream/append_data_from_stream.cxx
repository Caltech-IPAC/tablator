...
inline void insert_swapped(const size_t &column_offset, const Column &column,
                           ...
                           const size_t &old_position, Row &row) {
    return insert_swapped(column_offset, column.type, column.array_size, stream,
                          old_position, row);
}
    ...
    for (size_t r = 0; r < num_rows; ++r) {
        ...
        for (size_t column = 1; column < columns.size(); ++column) {
            if (is_null_MSb(stream, row_offset, column)) {
                row.set_null(columns[column].type, columns[column].array_size, column,
                ...
                if (fields[column].is_array_dynamic) {
                    ...
                    boost::spirit::qi::parse(begin, end, boost::spirit::qi::big_dword,
                                             dynamic_array_size);
                    insert_swapped(offsets[column], columns[column].type,
                                   dynamic_array_size, stream, position, row);
                    position += data_size(columns[column].type) * dynamic_array_size;
                } else {
