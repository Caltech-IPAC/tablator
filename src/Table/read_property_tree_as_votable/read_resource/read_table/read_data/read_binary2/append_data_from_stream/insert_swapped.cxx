...
void insert_swapped_element (std::vector<uint8_t>::const_iterator &begin,
                             ...
                             const size_t &offset, const bool &is_bool,
                             tablator::Row &row)
{
  switch (data_type_size)
    ...
    case 1:
      ...
      if (is_bool)
        {
          if (*begin != 't' && *begin != 'T' && *begin != '1' && *begin != 'f'
              && *begin != 'F' && *begin != '0')
            throw std::runtime_error ("Invalid value for boolean.  The "
      ...
      break;
    case 2:
      parse_and_insert_element<uint16_t>(boost::spirit::qi::big_word, begin,
                                         end, offset, row);
      break;
    case 4:
      parse_and_insert_element<uint32_t>(boost::spirit::qi::big_dword, begin,
                                         end, offset, row);
      break;
    case 8:
      parse_and_insert_element<uint64_t>(boost::spirit::qi::big_qword, begin,
                                         end, offset, row);
      break;
    default:
  ...
  for (size_t index = 0; index != array_size; ++index)
    {
      ...
      insert_swapped_element (begin, end, data_type_size,
                              column_offset + index * data_type_size, is_bool,
                              row);
    }
}
}
