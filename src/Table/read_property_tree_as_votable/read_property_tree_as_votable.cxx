...
inline void
throw_if_empty (const boost::property_tree::ptree::const_iterator &child,
    ...
    const boost::property_tree::ptree &tree)
{
  ...
  if (!votable_property.value.empty ()
      || !votable_property.attributes.empty ())
  ...
  while (child->first != "RESOURCE")
    {
      if (child->first == "COOSYS" || child->first == "PARAM"
        ...
        {
          throw std::runtime_error ("In VOTABLE, expected COOSYS, GROUP, "
                                    "PARAM, INFO, or RESOURCE, but got: "
                                    + child->first);
        }
      ++child;
      throw_if_empty (child, end);
