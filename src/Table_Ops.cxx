...
Table add_counter_column(const Table &src_table, const std::string &col_name) {
    ...
    // JTODO add Data_Details function to handle this?
    const auto &src_data = src_table.get_data();
    auto &dest_data = dest_table.get_data();
    dest_data.reserve(num_rows * dest_row_size);

    tablator::Row row(dest_row_size, src_table.get_num_dynamic_columns());
    uint64_t cntr(1);
    ...
    for (const auto *row_pointer = src_data.data();
        ...
        // No need to update dynamic_array_sizes_per_row.
...
Table combine_tables(const Table &src1_table, const Table &src2_table) {
    ...
    Row curr_row(dest_row_size, dest_table.get_num_dynamic_columns());
    for (size_t row_idx = 0; row_idx < num_rows; ++row_idx) {
        ...
        // Populate row.dynamic_array_sizes_.
        const std::vector<uint32_t> &src1_dynamic_array_sizes =
                src1_table.get_data_details().get_dynamic_array_sizes_by_row().at(
                        row_idx);
        const std::vector<uint32_t> &src2_dynamic_array_sizes =
                src2_table.get_data_details().get_dynamic_array_sizes_by_row().at(
                        row_idx);
        std::vector<uint32_t> &dest_dynamic_array_sizes =
                curr_row.get_dynamic_array_sizes();

        // Get dynamic array sizes from src1_table...
        dest_dynamic_array_sizes.insert(dest_dynamic_array_sizes.end(),
